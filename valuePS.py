...
scoreWrongAnswer = 0
scoreCorrectAnswer = 1.0
scoreBlankAnswer = 1.0/(numberAlternatives-1)
...
def valueOfAnswer(answer,probs):
    ...
    if not(indicatedAnswer.size): # there is no correct answer indicated => so answers is blank
        return scoreBlankAnswer
    else:   
        return sum(valueFunction([scoreCorrectAnswer])*answer*probs) + sum(valueFunction([scoreWrongAnswer])*answer*(1-probs))
...
## SIMULATION 1
# simulation with three answers that can be excluded so answer is [0,1,1,1]
answer = np.array([0,1,1,1]) # alles behalve antwoord A is geëlimineerd
probsAnswer = np.array([1,0,0.0,0.0])
print valueOfAnswer(answer, probsAnswer) # the value is the score of a correct answer


## SIMULATION 2
# simulation of doubt between two answers A and B [X,X,1,1]
#possibleAnswers = list(itertools.permutations([0,0,0,1]))
#possibleAnswers.append((0,0,0,0))
#possibleAnswers = list(set(possibleAnswers))


possibleAnswers = list(itertools.permutations([0,1]))
possibleAnswers = list(set(possibleAnswers))
possibleAnswers = [possibleAnswer + (0,0) for possibleAnswer in possibleAnswers]
possibleAnswers.append((0,0,0,0))
...
### SIMULATION 3
## simulation of doubt between two answers A and B [X,X,1,1]
possibleAnswers = list(itertools.permutations([0,0,1]))
possibleAnswers = list(set(possibleAnswers))
possibleAnswers = [possibleAnswer + (0,) for possibleAnswer in possibleAnswers]
possibleAnswers.append((0,0,0,0))
