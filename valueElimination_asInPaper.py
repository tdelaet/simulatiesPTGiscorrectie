...
wrongAnswer = -1
correctAnswer = 1.0/(numberAlternatives-1)
...
def valueOfAnswer(answer,probs):
    ...
    return sum(valueFunction([wrongAnswer])*answer*probs) + sum(valueFunction([correctAnswer])*answer*(1-probs))
...
possibleAnswers = cartesian(([0,1],[0,1],[1],[1])) # as a 2d matrix
...
possibleAnswers = cartesianList([[0,1],[0,1],[1],[1]])
...
## SIMULATION 3
# simulation of doubt between two answers A and B [X,X,1,1]
possibleAnswers = cartesian(([0,1],[0,1],[0,1],[1])) # as a 2d matrix
...
possibleAnswers = cartesianList([[0,1],[0,1],[0,1],[1]])
couplesPossibleAnswersProbs = cartesianList([possibleAnswers,probs])
values = valueOfAnswers(couplesPossibleAnswersProbs)
...
plt.savefig('maxValueAnswer_alpha'+str(alpha)+'_beta'+str(beta)+'.png')
...
plt.savefig('maxAnswer_alpha'+str(alpha)+'_beta'+str(beta)+'.png')
...
plt.savefig('maxValue_alpha'+str(alpha)+'_beta'+str(beta)+'.png')
