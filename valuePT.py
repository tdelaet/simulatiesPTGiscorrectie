...
nameMethod = 'PT'
# more difficult to simulate => one is supposed to follow the internal probabilities
# otherwise you should simulate how to deviate from internal probabilities
...
def valueOfAnswer(answer,probs):
    ...
    values = [ probs[i]*valueFunction([2*probs[i]-probs.dot(probs)])[0] for i in np.arange(len(probs))]    
    return sum(values)
